  ...
  const handleAllSelectChange = () => {
    if (props.selectedReportIds && props.setSelectedReportIds) {
      ...
      if (props.selectedReportIds && props.selectedReportIds.size === 0) {
        newSelectedReportIds = new Set(
          props.visibleReports.map((value) => {
            return value._id;
          })
        );
      } else {
    ...
    <Table bordered size='sm' className={"m-0"}>
      <thead>
        <tr>
          ...
          <th>Content</th>
          <th>Tags</th>
          {props.variant !== "group-details" && <th>Group</th>}
        </tr>
      </thead>
      <tbody>
        {props.visibleReports &&
          props.visibleReports.length > 0 &&
          props.variant !== "group-details" &&
          props.setSelectedReportIds &&
          props.selectedReportIds &&
...
export function ReportRow(props: ReportRowIProps) {
  ...
  const allGroupsQuery = useQuery<Group[] | undefined, AxiosError>(
    ["all-groups"],
    () => {
      return getAllGroups();
    },
      ...
      case "group-details":
        ...
        return (
          <tr
            ...
            className={
              props.report.read ? styles.reportRead : styles.reportUnread
            }
          >
            {props.variant !== "group-details" && (
            ...
            <td className={styles.td__sourceInfo}>
              <a
                ...
                target='_blank'
                className={styles.sourceInfo__link}
              >
      ...
      case "modal":
        return (
          <tr key={props.report._id}>
            ...
            <td className={styles.td__content + " text-break"}>
              <Link
                to={"/props.report/" + props.report._id}
                className={styles.content__link}
              >
                {/* This is a janky method of showing full tweets instead of shorted ones */}
                {reportFullContent(props.report) ? (
              ...
              {props.selectedGroup && (
                <>
                  ...
                  <span className={styles.group__totalReports}>
                    {props.selectedGroup._reports.length} reports
                  </span>
                  <br />
                  <span className={styles.group__idnum}>
                    ID: {props.selectedGroup.idnum}
                  </span>
                </>
              )}
              {!props.selectedGroup &&
          ...
          <tr key={props.report._id}>
            ...
            <td className={styles.td__sourceInfo}>
              ...
              <a
                href={props.report.url}
                target='_blank'
                className={styles.sourceInfo__source}
              >
                {capitalizeFirstLetter(props.report._media[0])}{" "}
                <FontAwesomeIcon
                  icon={faArrowUpRightFromSquare}
            ...
            <td className={styles.td__content + " text-break"}>
              ...
              <Form.Control
                ...
                disabled
                defaultValue={
                  props.report.notes ? props.report.notes : "No notes written."
                }
              />
            </td>
            <td className={styles.td__tags}>
        ...
        {placeholderValues.map((value) => {
          return (
            <tr key={"placeholderRow" + value}>
              ...
              <td className={styles.td__image}></td>
