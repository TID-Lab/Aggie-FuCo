...
export default function EditGroupModal(props: IProps) {
  ...
  const reportGroupUpdateMutation = useMutation(
    (reportGroupUpdateInfo: ReportGroupUpdateInfo) => {
      return setSelectedGroup(
        reportGroupUpdateInfo.reportIds,
        reportGroupUpdateInfo._group
      );
    },
    {
      onSuccess: (data) => {
                  ...
                  {groupById(props.groupId, allGroupsQuery.data) && (
                    <>
                      <span className={styles.group__title}>
                        {groupById(props.groupId, allGroupsQuery.data)?.title}
                      </span>
                      <br />
                      <span className={styles.group__totalReports}>
                        {groupById(props.groupId, allGroupsQuery.data)?._reports
                          .length === 1
                          ? groupById(props.groupId, allGroupsQuery.data)
                              ?._reports.length + " report"
                          : groupById(props.groupId, allGroupsQuery.data)
                      ...
                      <span className={styles.group__idnum}>
                        ID:{" "}
                        {groupById(props.groupId, allGroupsQuery.data)?.idnum}
                      </span>
                    </>
                  )}
                </Button>
